...
Fairly close! With even more iterations, it would approach the exact
probability...
...
I believe the former has a higher probability...
...
two_roll_results <- two_roll_results %>% 
  mutate(match_1 = ((die_1)^2 == die_2), 
         match_2 = abs(die_1 - die_2) == 4) 
two_roll_results
```

```{r summarize_rolls}
two_roll_results %>%
  count(match_1, match_2) %>%
  mutate(prop = n / sum(n))
```
